						...
						if(!$row_ip2){
							...
							if(is_numeric($config['kou_pr'])==true && $config['kou_pr']!==0){
								...
								if($row_user[0]['kou']<$config['kou_pr'] && $row_user[0]['kou']>=0){
									...
									$value_kou=arr2v($kou_arr);
									$mysql->query("insert into `koudata` values({$value_kou})");
									//注册多少天开始扣量
							...
							if($kouliang==0){
								...
								$value_referer=arr2v($arr_referer);
								$mysql->query("insert into `refererdata` values({$value_referer})");
